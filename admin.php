...
<script type="text/javascript">
  ...
  const ids = ['vao', 'vpo', 'vp', 'addcoup', 'cp', 'viewcou', 'dc', 'ap', 'rp', 'vc'];

  // Hide all sections first safely
  ids.forEach(id => {
  ...
  let showId = null;
  if (name == 'view_customers') showId = 'vc';
  else if (name == 'delete_customer') showId = 'dc';
  else if (name == 'add_product') showId = 'ap';
  else if (name == 'remove_product') showId = 'rp';
  else if (name == 'change_password') showId = 'cp';
  else if (name == 'view_products') showId = 'vp';
  else if (name == 'add_coupon') showId = 'addcoup';
  else if (name == 'view_all_orders') showId = 'vao';
  else if (name == 'view_pending_orders') showId = 'vpo';
  else if (name == 'view_coupon') showId = 'viewcou';
...
function hidefield() {
  const ids = ['vao', 'vpo', 'dc', 'vp', 'ap', 'viewcou', 'rp', 'addcoup', 'vc', 'cp'];
  ids.forEach(id => {
    const el = document.getElementById(id);
    if (el) el.style.display = 'none';
  });
...
function addNewSizeRow() {
  ...
  newRow.innerHTML = `
    <input type="text" name="sizes[]" placeholder="Size (e.g., US 7)" required>
  ...
  `;
  document.getElementById('size-fields').appendChild(newRow);
}
	  ...
	  <form method="POST" action="add_product.php" enctype="multipart/form-data">
		  <center>
				...
				<div class="size-fields">
					<label class="size-label">Shoe Sizes & Quantities:</label>
					<div id="size-fields">
						<div class="size-row">
							<input type="text" name="sizes[]" placeholder="Size (e.g., US 7)" required>
							<input type="number" name="quantities[]" placeholder="Quantity" min="1" required>
    ...
    <?php
      ...
      else
      {
        while($row = $result->fetch_assoc())
        {
          ...
          $sizes_query = "SELECT size, stock_quantity FROM product_sizes WHERE product_id = $prid";
          $sizes_result = mysqli_query($conn, $sizes_query);
          $sizes_list = [];
          while($size_row = $sizes_result->fetch_assoc()) {
            $sizes_list[] = $size_row['size'] . ' (Qty: ' . $size_row['stock_quantity'] . ')';
          }
          $sizes_display = !empty($sizes_list) ? implode(', ', $sizes_list) : 'No sizes';
          ...
          $pr_sizes='<a href="product_edit.php?productid='.$prid.'&"><font color=rgb(255,255,255)>'.$sizes_display.'</a></font>';
          
          echo '<tr><td>'.$pr_id.'</td><td>'.$pr_n.'</td><td>'.$pr_cid.'</td><td>'.$pr_cn.'</td><td>'.$pr_pr.'</td><td>'.$pr_des.'</td><td>'.$pr_sizes.'</td></tr>';
